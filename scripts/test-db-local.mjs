...
if (!url)
  throw new Error(
    "Set BRD_TEST_DATABASE_URL to an empty, disposable local PostgreSQL database.",
  );
...
try {
  const { rows } = await client.query(
    "select count(*)::int as n from information_schema.tables where table_schema in ('public', 'auth')",
  );
  if (rows[0].n !== 0)
    throw new Error(
      "Refusing to bootstrap a non-empty database. Create a fresh local database.",
    );
  await client.query(
    await readFile("supabase/tests/local_bootstrap.sql", "utf8"),
  );
  for (const name of (await readdir("supabase/migrations"))
    ...
    if (name === "0024_reporting_history.sql") {
      ...
      await client.query(
        await readFile(
          "supabase/tests/price_history_upgrade_before.sql",
          "utf8",
        ),
      );
      ...
      await client.query(
        await readFile(
          "supabase/migrations/0026_preserve_price_history.sql",
          "utf8",
        ),
      );
      await client.query(
        await readFile(
          "supabase/tests/price_history_upgrade_after.sql",
          "utf8",
        ),
      );
    ...
    if (name === "0013_location_access.sql") {
      await client.query("begin");
      await client.query(
        await readFile("supabase/tests/location_upgrade_before.sql", "utf8"),
      );
      await client.query(await readFile(`supabase/migrations/${name}`, "utf8"));
      await client.query(
        await readFile("supabase/tests/location_upgrade_after.sql", "utf8"),
      );
      await client.query("rollback");
      console.log(
  ...
  for (const file of [
    ...
    "imports.sql",
    "business_logos.sql",
    "bulk_count_override.sql",
    "stock_counts.sql",
    "module_access.sql",
    "cash_up.sql",
    "order_workflow.sql",
    "document_workflows.sql",
    "employee_invitations.sql",
    "batch_expiry_daily_totals.sql",
    "cash_shifts_barcode.sql",
    "invoice_refinements.sql",
    "store_currencies_documents.sql",
    "product_descriptions.sql",
    "detailed_permissions.sql",
    "smtp_delivery.sql",
