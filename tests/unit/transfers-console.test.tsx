...
describe("transfer capture", () => {
  ...
  function addItem() {
    ...
    fireEvent.click(screen.getByText("Source product"));
    fireEvent.click(screen.getByText("Matching destination product"));
    fireEvent.click(screen.getByRole("button", { name: "Add item" }));
  }
  it("keeps the same request ID when retrying an uncertain save", async () => {
    ...
    expect(mock.rpc.mock.calls[0][1].p_request).toBe(
      mock.rpc.mock.calls[1][1].p_request,
    );
    expect(mock.rpc.mock.calls[0][1].p_items).toEqual([
      {
        source_product_id: "source-product",
        destination_product_id: "destination-product",
        quantity: 1,
      },
    ]);
  });
