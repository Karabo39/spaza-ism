...
it("reduces four customer keystrokes to one request and preserves the new-customer form", async () => {
  ...
  render(
    <QueryClientProvider client={client}>
      <CustomerPicker open onOpenChange={vi.fn()} onSelect={vi.fn()} />
    </QueryClientProvider>,
  );
  await act(async () => {
...
it("still runs the offline-aware product lookup without a connection", async () => {
  ...
  render(
    <QueryClientProvider client={client}>
      <ProductSearchDialog open onOpenChange={vi.fn()} onPick={vi.fn()} />
    </QueryClientProvider>,
  );
  await act(async () => {
...
it("coalesces a typing burst into one search and resets on reopening", async () => {
  ...
  lookup.mockClear();
  const input = screen.getByPlaceholderText(/Search name, SKU/);
  ...
  rerender(view(true));
  expect(
    (screen.getByPlaceholderText(/Search name, SKU/) as HTMLInputElement).value,
  ).toBe("");
});
