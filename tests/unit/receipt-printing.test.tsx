...
vi.mock("@/features/billing/print-receipt", () => ({
  PrintReceipt: ({ onPrint }: { onPrint: () => void }) => (
...
it("starts printing only once when opened from the sale prompt", async () => {
  mocks.rpc.mockResolvedValue({ error: null });
  const print = vi.spyOn(window, "print").mockImplementation(() => {});
  await act(async () => {
    render(
  ...
  expect(print).toHaveBeenCalledTimes(1);
  expect(mocks.rpc).toHaveBeenCalledExactlyOnceWith("record_receipt_print", {
    p_sale: "saved-sale",
    p_action: "REQUESTED",
  });
});
