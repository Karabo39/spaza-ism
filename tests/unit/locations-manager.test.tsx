...
vi.mock("@/lib/offline/offline-context", () => ({
  useOffline: () => ({ online: mocks.online }),
}));
...
describe("location creation", () => {
  beforeEach(() => {
    ...
    mocks.owner = true;
    mocks.online = true;
    mocks.rpc.mockResolvedValue({ data: "warehouse", error: null });
  });
  it("creates a warehouse through the audited RPC and refreshes available locations", async () => {
    render(<LocationsManager locationType="warehouse" />);
    ...
    await waitFor(() =>
      expect(mocks.rpc).toHaveBeenCalledWith("create_location", {
        p_business: "business",
