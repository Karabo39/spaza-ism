...
beforeEach(()=>{mock.rpc.mockReset();mock.fetch.mockReset();mock.session.mockResolvedValue({activeStore:{id:store,name:"Shop",businessName:"Business",currency:"ZAR",modules:{reports:true}},fullName:"Cashier"});vi.stubEnv("RESEND_API_KEY","test-key");vi.stubEnv("REPORT_EMAIL_FROM","Reports <reports@example.test>");vi.stubGlobal("fetch",mock.fetch);});
...
it("rejects email when Reports is disabled for this store",async()=>{mock.session.mockResolvedValue({activeStore:{id:store,modules:{reports:false}}});expect((await POST(request())).status).toBe(403);expect(mock.rpc).not.toHaveBeenCalled();expect(mock.fetch).not.toHaveBeenCalled();});
