...
export async function reportXlsx(data:ExportData):Promise<ArrayBuffer>{
  ...
  if(data.createdAt){const {default:JSZip}=await import("jszip");const zip=await JSZip.loadAsync(buffer);zip.forEach((_path,entry)=>{entry.date=created;});return zip.generateAsync({type:"arraybuffer",compression:"DEFLATE"});}
