...
export function InvoiceWorkspace({
  ...
  account: { balance: number; credit_limit: number };
}) {
  const { can } = useStore();
  ...
  function post() {
    ...
    const payload = {
      ...
      p_amount: cents! / 100,
      ...(kind === "PAYMENT"
        ? { p_method: method, p_reference: reference }
        : {}),
      p_reason: reason,
    };
    void run(
      () =>
        createClient().rpc("post_invoice_entry", {
          ...payload,
          p_request: request(payload),
        }),
      kind === "PAYMENT" ? "Payment recorded" : "Note posted",
      () => {
        setAmount("");
        setReason("");
        setReference("");
        ...
        <TBody>
          {items.map((l) => (
            <TR key={l.id}>
              ...
              <TD>{money(l.unit_price, i.currency)}</TD>
              <TD>{money(l.line_total, i.currency)}</TD>
            </TR>
          ))}
        </TBody>
      </Table>
        ...
        <div className="rounded-lg border border-border bg-surface p-5 space-y-3">
          ...
          <Button
            ...
            onClick={() =>
              run(
                () =>
                  createClient().rpc("issue_sales_invoice", {
                    p_invoice: i.id,
                  }),
                "Invoice issued",
              )
      ...
      {settled && (
        ...
        >
          ...
          {can("manager") && (
            ...
            >
              {showAdjustments ? "Close adjustment" : "Make adjustment"}
            </Button>
          )}
        </section>
      )}
        ...
        (!settled || (can("manager") && showAdjustments)) && (
          <section className="rounded-lg border border-border bg-surface p-5 space-y-3">
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              <div>
                <Label htmlFor="entry-kind">Transaction</Label>
                <select
                  id="entry-kind"
            ...
            {kind === "PAYMENT" ? (
              <>
                ...
                <p className="text-xs text-muted">
                  Credit terms leave the balance outstanding until money is
                  received.
                </p>
              </>
            ) : (
              <p className="text-xs text-muted">
                Financial notes adjust the amount owed. Use Return goods for
                physical returns and refunds.
              </p>
            )}
            <Label htmlFor="entry-reason">
              {kind === "PAYMENT" ? "Note (optional)" : "Reason (required)"}
            </Label>
            <Input
              id="entry-reason"
              value={reason}
          ...
          {needsApproval &&
            (can("manager") ? (
              <label className="flex gap-2 text-sm">
                ...
                />
                Approve release above the credit limit
              </label>
            ) : (
              <OverrideApproval
                customerId={i.customer_id}
                amount={Number(i.outstanding)}
                onApproved={setToken}
              />
            ))}
          {token && (
            <p className="text-sm text-success">
              Approved. Release goods within two minutes.
            </p>
          )}
          <Button
            loading={busy}
            disabled={
              !online ||
              (i.terms !== "CREDIT" && Number(i.outstanding) > 0) ||
              Number(i.credits) > 0
            }
            onClick={() =>
              run(
                () =>
                  createClient().rpc("issue_invoice_goods", {
                    p_invoice: i.id,
                    p_override: override,
                    ...(token ? { p_override_token: token } : {}),
                  }),
                "Goods released",
              )
            }
          >
            Release invoice goods
          </Button>
        </section>
      )}
      {i.state === "ISSUED" && Number(i.outstanding) > 0 && (
      ...
      {can("manager") &&
        ...
        !i.goods_issued_at && (
          <div className="flex gap-3">
            <Input
              aria-label="Invoice cancellation reason"
              placeholder="Reason for cancellation / void"
              value={reason}
              onChange={(e) => setReason(e.target.value)}
            />
            <Button
              ...
              onClick={() =>
                run(
                  () =>
                    createClient().rpc("cancel_sales_invoice", {
                      p_invoice: i.id,
                      p_reason: reason,
                    }),
