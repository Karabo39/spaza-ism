...
export function LocationsManager({
  ...
  return (
    <Card className="mt-5" id="new-location">
      <CardHeader>
        <CardTitle>
          {type === "warehouse" ? "Add Warehouse" : "Add Store"}
        </CardTitle>
        <CardDescription>
          Each location has its own stock and stock take. Warehouses hold stock
          and cannot record sales.
        </CardDescription>
      </CardHeader>
      <CardContent
        className={activateOnCreate ? "max-w-2xl" : "grid gap-6 lg:grid-cols-2"}
      >
        {!activateOnCreate && (
          <ul className="divide-y divide-border">
            {stores
              .filter(
                (s) =>
                  s.businessId === store.businessId && s.locationType === type,
              )
              .map((s) => {
