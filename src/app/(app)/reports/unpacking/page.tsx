...
export default async function UnpackingReport({
  searchParams,
}: {
  ...
  const columns = [
    ...
    { key: "unit_name", label: "Unit product" },
    { key: "units_per_pack", label: "Ratio" },
    { key: "packs", label: "Packs removed" },
    { key: "units", label: "Units added" },
    { key: "date", label: "Date" },
  ...
  return (
    <>
      <PageHeader
        title="Unpacking Report"
        description="Linked bulk-pack decreases and unit-stock increases, with conversion and reason."
        crumbs={[
        ...
        actions={
          <>
            <DateFilter />
            <ExportButton rows={rows} columns={columns} filename="unpacking" />
          </>
        }
      />
      {rows.length === 1000 && (
        <p className="text-warning">
          Latest 1,000 unpackings; narrow the date range for a complete report.
        </p>
      )}
