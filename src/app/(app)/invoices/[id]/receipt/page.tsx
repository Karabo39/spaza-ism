...
export default async function ReceiptPage({
  ...
  const session = await getSession("invoices_view_invoices");
    ...
    <>
      ...
      >
        ...
        <div className="grid gap-2 sm:grid-cols-2">
          ...
          <p>Issued: {i.issued_at ? dateTime(i.issued_at) : "Draft"}</p>
          <p>Due: {dateOnly(i.due_date)}</p>
          <p>Status: {i.status.replaceAll("_", " ")}</p>
          <p>
            Goods:{" "}
            {i.goods_issued_at
              ? `Released ${dateTime(i.goods_issued_at)}`
              : "Awaiting collection"}
          </p>
        </div>
          ...
          <tbody>
            {lines.data?.map((l) => (
              <tr key={l.id} className="border-b border-gray-200">
                <td className="py-3">{l.product_name}</td>
        ...
        <div className="text-right space-y-1">
          ...
          <p>
            Tax ({i.tax_percent}%): {money(i.tax_amount, i.currency)}
          </p>
          <p className="font-bold text-xl">
            Invoice total: {money(i.total, i.currency)}
          </p>
          <p>
            Debit notes: {money(i.debits, i.currency)} · Credit notes:{" "}
            {money(i.credits, i.currency)}
          </p>
          <p>Received: {money(i.paid, i.currency)}</p>
          <p className="font-bold">
            Outstanding: {money(i.outstanding, i.currency)}
          </p>
        </div>
        <section>
          <h3 className="font-semibold">Payments and adjustments</h3>
          {entries.data
            ?.filter((e) => e.kind !== "ISSUE")
            .map((e) => (
              <div key={e.id} className="py-2 border-b border-gray-200 text-xs">
                <p>
                  {dateTime(e.created_at)} · {e.kind.replaceAll("_", " ")} ·{" "}
                  {money(e.amount, i.currency)} · {e.method?.replace("_", "/")}
                </p>
                <p>
                  {e.reference} {e.payment_reference} {e.reason}
                </p>
              </div>
            ))}
        </section>
        {i.note && <p>{i.note}</p>}
        <p className="text-xs">Keep this document for queries or returns.</p>
      </article>
    </>
  );
}
