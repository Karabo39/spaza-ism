...
export default async function StockTakePage({
  ...
  const { data } = await supabase
    .from("stock_takes")
    .select("id, status, note, created_at, completed_at")
  ...
  return (
    <>
      <PageHeader
        title="Stock Take"
        crumbs={[{ label: "Stock Control" }, { label: "Stock Take" }]}
            ...
            <TBody>
              {rows.map((r) => (
                <TR key={r.id}>
                  <TD>
                    <Link
                      href={`/warehouse/${warehouseId}/stock-take/${r.id}`}
                  ...
                  <TD className="text-muted">
                    {r.completed_at ? dateTime(r.completed_at) : "—"}
                  </TD>
